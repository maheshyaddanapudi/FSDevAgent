...
import EnhancedUnifiedEmulator from '../components/UnifiedEmulator/EnhancedUnifiedEmulator';
...
const ChatPage = () => {
  ...
  const [isEmulatorMinimized, setIsEmulatorMinimized] = useState(false);
  
  // Refs for auto-scroll
  const messagesEndRef = useRef(null);
  const inputRef = useRef(null);
  
  // Enhanced chat state management with error handling
  const { 
    ...
    // Human-in-the-loop state and actions
  ...
  // Handle input changes for dynamic button behavior
  useEffect(() => {
  ...
  }, [handleSendMessage, handleStopAgent, isProcessing, isTyping]);
  ...
  }, [clearChat, setAiDeveloperAgentSessionId]);
  ...
  // Handle emulator minimize toggle
  const handleEmulatorMinimizeToggle = useCallback((minimized) => {
    setIsEmulatorMinimized(minimized);
  }, []);

  // Determine button state and text
  const getButtonState = () => {
    if (waitingForHumanInput) {
      return { text: '📤', mode: 'human-input', disabled: !message.trim() };
    }
    if (isProcessing && !isTyping) {
      return { text: '⏹️ Stop', mode: 'stop', disabled: false };
    }
    if (isProcessing && isTyping) {
      return { text: 'Send', mode: 'send', disabled: !message.trim() };
    }
    return { text: 'Send', mode: 'send', disabled: !message.trim() };
  };
  ...
  // Mock current plan for demonstration
  const currentPlan = {
    title: "Current Task Plan",
    steps: [
      { id: 1, text: "Analyze user request", status: "completed" },
      { id: 2, text: "Plan implementation approach", status: "completed" },
      { id: 3, text: "Execute development tasks", status: isProcessing ? "current" : "pending" },
      { id: 4, text: "Test and validate results", status: "pending" },
      { id: 5, text: "Deliver final output", status: "pending" }
    ]
  };
  ...
  return (
    <div className="chat-page">
      ...
      <div className="main-content">
            ...
            <div className={`chat-input ${waitingForHumanInput ? 'human-input-mode' : ''}`}>
              <input
                ...
                placeholder={
                  ...
                  isProcessing ? 'Type to interrupt or press Enter to stop...' :
                  'Type your message...'
                }
                disabled={isInterfaceDisabled}
              ...
              <button 
                onClick={
                  waitingForHumanInput ? 
                    ...
                    buttonState.mode === 'stop' ? handleStopAgent : handleSendMessage
                }
                disabled={isInterfaceDisabled || buttonState.disabled}
                className={`send-button ${buttonState.mode}`}
              >
                {buttonState.text}
              </button>
            </div>
        ...
        {/* Right Half - Enhanced Emulator with Plan Display */}
        <div className={`emulator-section ${isEmulatorMinimized ? 'minimized' : 'expanded'}`}>
          <EnhancedUnifiedEmulator 
            toolOutputs={toolOutputs}
            currentPlan={currentPlan}
            isAgentWorking={isProcessing}
            onMinimizeToggle={handleEmulatorMinimizeToggle}
          />
        </div>
      </div>
    </div>
