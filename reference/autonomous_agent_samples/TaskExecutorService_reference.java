...
public class TaskExecutorService {
     ...
     */
    private Flux<ToolOutput> executeToolSequence(List<ToolInvocation> sequence, Map<String, Object> context) {
        return Flux.fromIterable(sequence)
            .concatMap(invocation -> {
                Tool tool = toolRegistry.getTool(invocation.toolName);
                if (tool == null) {
                    return Flux.just(ToolOutput.builder()
                            .type("error")
                            .content("Tool not found: " + invocation.toolName)
                            .build());
                }
                
                // Add context to arguments
                Map<String, Object> args = new HashMap<>(invocation.arguments);
                args.putAll(context);
